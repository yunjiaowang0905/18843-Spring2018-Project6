...
def get_pre_slt(file_path, date_min, date_max, gas_max_val, car_no, lon_min, lon_max, lat_min, lat_max, n_lat, n_lon):
    """
    ...
    # area selection
    data_q_idx = np.nonzero((data_mat[:,2] >= lat_min)\
        & (data_mat[:,2] <= lat_max)\
        & (data_mat[:,3] >= lon_min)\
        & (data_mat[:,3] <= lon_max))
    data_q = data_t[data_clip_idx[0],:]

    xi = np.linspace(lon_min, lon_max, num=n_lon)
    yi = np.linspace(lat_min, lat_max, num=n_lat)
    #TODO:test
    xr = interp1d(xi, data_q[:,3], kind="nearest" ,aixs=len(xi))
    yr = interp1d(yi, data_q[:,2], kind="nearest" ,aixs=len(yi))
    ...
    data_slt_idx = np.nonzero(hour(data_q[:, 2]) >= t_min & hour(data_q[:, 2]) < t_max)
    data_slt = data_q[data_slt_idx[0],:]
